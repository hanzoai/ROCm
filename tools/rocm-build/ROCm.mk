...
$(foreach comp,$(strip $2),$(call peval,${OUT_DIR}/logs/Signatures/${1}: ${OUT_DIR}/logs/Signatures/${comp}))
)
endef
...
rocm-dev-components :=amd_smi_lib aqlprofile aqlprofiletest comgr dbgapi devicelibs hip_on_rocclr hipcc hipify_clang \
	kernel_ubuntu lightning rocprofiler-compute opencl_on_rocclr openmp_extras rocm_bandwidth_test rocm_smi_lib \
...
$(call adddep,hipsparselt,hip_on_rocclr hipsparse lightning hipcc openmp_extras roctracer)
$(call adddep,hiptensor,hip_on_rocclr composable_kernel lightning hipcc)
$(call adddep,miopen-hip,rocm-core composable_kernel half hip_on_rocclr hipblas hipblaslt rocrand roctracer lightning hipcc)
...
$(call adddep,rocshmem,rocm-cmake rocthrust rccl)
...
$(call adddep,rpp,half lightning hipcc openmp_extras hipify_clang)
$(call adddep,transferbench,rocm-cmake hip_on_rocclr lightning hipcc)
ifneq ($(filter rocm-dev upload-rocm-dev, ${MAKECMDGOALS}),)
	components = $(rocm-dev-components)
endif
$(call adddep,rocm,$(filter-out ${NOBUILD} kernel_ubuntu rocm,${components}))
...
# parallel build
${OUT_DIR}/logs/$1.txt: ${OUT_DIR}/logs/Signatures/$1
...
${OUT_DIR}/logs/Signatures/$1: | ${OUT_DIR}/logs/Signatures
	@+${INFRA_REPO}/gensig $1 \
	$$(sort $$^ ${INFRA_REPO}/envsetup.sh ${INFRA_REPO}/compute_utils.sh ${INFRA_REPO}/build_$1.sh) \
	> $$@.tmp ; cmp -s $$@.tmp $$@ || { mv -f $$@.tmp $$@; echo $$@ updated ; }
